...
def plt_roast(X, Y):
    ...
    ax.scatter(
        X[Y == 1, 0], X[Y == 1, 1], s=70, marker="x", c="red", label="Torrefação Boa"
    )
    ax.scatter(
        X[Y == 0, 0],
        X[Y == 0, 1],
    ...
    ax.set_title(f"Torrefação de café", size=16)
...
def plt_layer(X, Y, W1, b1, norm_l):
    ...
    for i in range(W1.shape[1]):
        ...
        ax[i].set_title(f"Camada 1, unidade {i}")
        ax[i].set_xlabel("Temperatura \n(Celsius)", size=12)
    ax[0].set_ylabel("Duração \n(minutos)", size=12)
    plt.show()
...
def plt_network(X, Y, netf):
    ...
    ax[0].set_title(f"probabilidade da rede")
    ...
    ax[1].scatter(
        ...
        edgecolors=dlc["dldarkblue"],
        linewidth=1,
        label="Torrefação Ruim Prevista",
    )
    ax[1].set_title(f"Decisão da rede")
    ax[1].set_xlabel("Temperatura \n(Celsius)", size=12)
    ax[1].set_ylabel("Duração \n(minutes)", size=12)
...
def plt_output_unit(W, b):
    ...
    ax.set_xlabel("unidade 0")
    ax.set_ylabel("unidade 1")
    ax.set_zlabel("unidade 2")
    ax.view_init(30, -120)
    ax.figure.colorbar(pcm, ax=ax)
    ax.set_title(f"Camada 2, unidade de saída")
