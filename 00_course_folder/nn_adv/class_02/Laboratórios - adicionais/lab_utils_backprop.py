...
def between(a, b, x):
    """determinar se um ponto x está entre a e b. a pode ser maior ou menor que b"""
    if a > b:
        return b <= x <= a
    if b > a:
        return a <= x <= b
...
def near(pt, alist, dist=15):
    for a in alist:
        x, y = (
            a.ao.get_position()
        )  # (bot left, bot right) coordenadas de dados, não relativas
        x = x - 5
        y = y + 2.5
...
def inboxes(pt, boxlist):
    """retorna true se pt estiver dentro de uma das caixas em boxlist"""
    # with out:
...
class avalue:
    """um dos valores da figura que pode ser preenchido"""
...
class astring:
    """uma string que pode ser definida como visível ou invisível"""
...
class abox:
    """uma das caixas no gráfico que tem um valor"""

    def __init__(self, ax, value, left, bottom, right, top, anpt, cl, adj_anote_obj):
        self.ax = ax
        self.value = value  # valor correto para a anotação
        self.left = left
        self.right = right
        self.bottom = bottom
        self.top = top
        self.anpt = anpt  # x,y onde a expressão deve ser listada
        self.cl = cl
        self.ao = self.ax.annotate("?", self.anpt, c=self.cl, fontsize="x-small")
        self.astr = adj_anote_obj  # 2º texto secundário para marcar bordas ou nenhum
    ...
    def show_secondary(self):
        if self.astr:  # se houver um segundo conjunto de texto secundário
...
## Para depuração, coloque isso no notebook que está sendo depurado e certifique-se de definir o parâmetro out=out
...
class plt_network:
    ...
    def __init__(self, fn, image, out=None):
        ...
        self.ax.imshow(img)
        self.fig.text(0.1, 0.9, "Clique nas caixas para preencher os valores.")
        self.glist = []  # lugar para guardar coisas globais
        self.san = []  # anotação selecionada
        ...
        self.breveal = Button(self.axreveal, "Revelar\ntodos")
        self.breveal.on_clicked(self.reveal_values)
        self.bhide = Button(self.axhide, "Esconder\ntodos")
        self.bhide.on_clicked(self.hide_values)
        # plt.show()

    def sizefig(self, img):
        iy, ix, iz = np.shape(img)
        if 10 / 5 < ix / iy:  # se x for o tamanho limite
            figx = 10
            figy = figx * iy / ix
        else:
            figy = 5
    ...
    def onclick(self, event):
        ...
        if inbox:
            ...
            txtBox = TextBox(graphBox, "novo valor: ")
...
def config_nw0(ax):
    ...
    box1 = abox(
        ax, round(a, 2), 307, 140, 352, 100, (315, 128), "blue", None
...
def config_nw1(ax):
    ...
    box1 = abox(
        ax, round(c, 2), 330, 162, 382, 114, (338, 150), "blue", None
    )  # esquerda, inferior, direita, superior,
