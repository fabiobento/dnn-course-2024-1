...
def plt_act_trio():
    ...
    ax[2].set_title("ReLu")
    fig.suptitle("Funções de Ativação Comuns", fontsize=14)
...
def plt_act1(X, y, z, a):
    ...
    ax[0].axvline(0, lw=0.3, c="black")
    ax[0].axhline(0, lw=0.3, c="black")
    ax[0].set_title("y - alvo")
    ax[1].plot(X, y, label="alvo")
    ...
    ax[2].legend()
    fig.suptitle("Função da Ativação Não Linear", fontsize=12)
...
def plt_model(X, y, yhat_pre, yhat_post):
    ...
    ax[0].set_ylabel("y")
    ax[0].plot(X, y)
    ax[1].set_title("Predição, pre-treino")
    ax[1].set_xlabel("x")
    ax[1].set_ylabel("y")
    ax[1].plot(X, y)
    ax[1].plot(X, yhat_pre)
    ax[2].set_title("Predição, pos-treino")
    ax[2].set_xlabel("x")
    ax[2].set_ylabel("y")
    ax[2].plot(X, y)
...
def display_errors(model, X, y):
    ...
    if len(idxs) == 0:
        ...
        for i in range(cnt):
            ...
            # Predizer usando a rede neural
            prediction = model.predict(X[j].reshape(1, 400))
            prediction_p = tf.nn.softmax(prediction)
            yhat = np.argmax(prediction_p)

            # Exibir o rótulo abaico da imagem
            ax[i].set_title(f"{y[j,0]},{yhat}", fontsize=10)
            ax[i].set_axis_off()
            fig.suptitle("Rótulo, yhat", fontsize=12)
    return len(idxs)


def display_digit(X):
    """Exibir um único dígito. A entrada é um único dígito (400,)."""
...
def plot_loss_tf(history):
    ...
    widgvis(fig)
    ax.plot(history.history["loss"], label="perda")
    ax.set_ylim([0, 2])
    ax.set_xlabel("Época")
