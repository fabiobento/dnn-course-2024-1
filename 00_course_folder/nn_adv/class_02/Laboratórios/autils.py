...
def plt_act1(X, y, z, a):
    ...
    ax[2].plot(X, y, label="alvo")
    ax[2].plot(X, a, c=dlc["dldarkred"], label="ReLu(z)")
    ax[2].axhline(0, lw=0.3, c="black")
    ax[2].axvline(0, lw=0.3, c="black")
    ax[2].set_title("max(0,z)")
    ax[2].legend()
    fig.suptitle("Papel da Ativação Não Linear", fontsize=12)
...
def plt_model(X, y, yhat_pre, yhat_post):
    ...
    ax[0].set_ylabel("y")
    ax[0].plot(X, y)
    ax[1].set_title("Predição, pre-treinamento")
    ax[1].set_xlabel("x")
    ax[1].set_ylabel("y")
    ax[1].plot(X, y)
    ax[1].plot(X, yhat_pre)
    ax[2].set_title("Predição, pós-treinamento")
    ax[2].set_xlabel("x")
    ax[2].set_ylabel("y")
    ax[2].plot(X, y)
...
def display_digit(X):
    """exibe um único dígito. A entrada é um dígito (400,)."""
