...
def plt_relu_ex():
    ...
    fig = plt.figure()
    fig.suptitle("Explore a Ativação Não-Linear")
    ...
    ax[0].set_title(" Corresponder ao alvo ")
    ax[0].legend()
    ax[0].set_xlabel("x")
    ax[1].set_title("Unidade 0 (fixada) ")
    ax[1].legend()
    ax[2].set_title("Unidade 1")
    ax[2].legend()
    ax[3].set_title("Unidade 2")
    ax[3].legend()
    plt.tight_layout()
    ...
    return [
        ...
        sb2,
        artists,
    ]  # retornou para manter uma referência ativa aos controles deslizantes
