...
def gradient_descent(
    ...
    for i in range(num_iters):
        ...
        if i % math.ceil(num_iters / 10) == 0:
            if verbose:
                print(f"Iteração {i:4d}: Cost {ccost}   ")
            if verbose == 2:
                print(f"dj_db, dj_dw = {dj_db: 0.3f}, {dj_dw.reshape(-1)}")

            if ccost == last_cost:
                alpha = alpha / 10
                print(f" alpha agora {alpha}")
            last_cost = ccost
...
def plt_tumor_data(x, y, ax):
    """plota os dados do tumor em um eixo"""
    pos = y == 1
    neg = y == 0
    ...
    ax.set_title("Regressão logística em Dados Categóricos")
...
class button_manager:
    """Lida com alguns recursos ausentes dos botões de verificação do matplotlib
    na inicialização:
        cria o botão, vincula a rotina button_click,
        chama call_on_click com índice ativo e firsttime=True
    no clique:
        mantém um único botão no estado, chama call_on_click
    """
    ...
    def reinit(self):
        ...
        )  # desativar o antigo, acionará a atualização e definirá o status

    # @output.capture()  # debug
    def button_click(self, event):
        """mantém o estado de ligado. Se o botão de ligar for clicado, ele será processado corretamente"""
