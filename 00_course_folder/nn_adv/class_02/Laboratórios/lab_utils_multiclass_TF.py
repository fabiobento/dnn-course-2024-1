...
# Traçar um limite de decisão categórica multiclasse
# Essa versão lida com uma previsão não vetorial (adiciona um loop for sobre os pontos)
def plot_cat_decision_boundary_mc(
    ax, X, predict, class_labels=None, legend=False, vector=True
):

    # criar uma malha para os pontos a serem plotados
    x_min, x_max = X[:, 0].min() - 0.5, X[:, 0].max() + 0.5
    y_min, y_max = X[:, 1].min() - 0.5, X[:, 1].max() + 0.5
    h = max(x_max - x_min, y_max - y_min) / 100
    xx, yy = np.meshgrid(np.arange(x_min, x_max, h), np.arange(y_min, y_max, h))
    points = np.c_[xx.ravel(), yy.ravel()]
    # print("pontos", points.shape)
    ...
    # O gráfico de contorno destaca os limites entre os valores - classes, neste caso
...
def plt_mc_data(
    ...
    """Plotar dados multiclasse. Observe que, se equal_xy for True, a definição de ylim no gráfico pode não funcionar"""
...
def plt_mc(X_train, y_train, classes, centers, std):
    ...
    ax.set_title("Dados Multiclasse")
    ax.set_xlabel("x0")
    ax.set_ylabel("x1")
    # for c in css:
...
def plt_cat_mc(X_train, y_train, model, classes):
    # Criar um modelo para chamar as rotinas de plotagem
    ...
    # Adicionar os dados originais ao limite da decisão
    ...
    ax.set_title("fronteira de decisão do modelo")
...
def plt_prob_z(ax, fwb, x0_rng=(-8, 8), x1_rng=(-5, 4)):
    """traça um limite de decisão, mas inclui sombreamento para indicar a probabilidade
    e adiciona um contador para mostrar onde z=0
    """
    # Configuração de intervalos úteis e espaços de linha comuns
    x0_space = np.linspace(x0_rng[0], x0_rng[1], 40)
    x1_space = np.linspace(x1_rng[0], x1_rng[1], 40)

    # Obter a probabilidade dos intervalos x0,x1
    ...
    for i in range(tmp_x0.shape[0]):
        for j in range(tmp_x1.shape[1]):
            ...
            z[i, j] = fwb(x)
            c[i, j] = 0.0 if z[i, j] == 0 else 1.0
    with warnings.catch_warnings():  # Suprimir aviso de ausência de contorno
...
def plt_layer_relu(X, Y, W1, b1, classes):
    ...
    for i in range(nunits):
        ...
        ax[i].set_title(f"Unidade {i} da camada 1")
...
def plt_output_layer_linear(X, Y, W, b, classes, x0_rng=None, x1_rng=None):
    ...
    for i, axi in enumerate(ax.flat):
        ...
        axi.set_xlim(x0_rng)
        axi.set_ylim(x1_rng)
        axi.set_title(f"Saída Linear da Unidade {i}")
    fig.tight_layout()
    plt.show()
