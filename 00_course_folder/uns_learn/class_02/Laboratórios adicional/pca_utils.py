...
def plot_widget_2(doc):
    ...
    main_scatter = main_plot.scatter(
        X[:, 0], X[:, 1], marker="o", size=4, color="#C00000"
    )

    ## PCA line (static)
    pca_line_plot = line_between_points(
        main_plot,
        pca_line[0],
        pca_line[1],
        color="#333333",
        line_width=1.5,
        legend_label="PCA Line",
    )

    ## Rotating line initial state:
    rotating_line_initial_state = [(-1.5, 0), (1.5, 0)]

    rotating_line = line_between_points(
        main_plot,
    ...
    projection_points = main_plot.scatter(
        orthogonal_set[:, 0],
        orthogonal_set[:, 1],
        marker="x",
        size=10,
    ...
    for p, o in zip(X, orthogonal_set):
        lines.append(
            line_between_points(main_plot, p, o, color="#FF9300", line_width=1)
        )
...
def plot_3d_2d_graphs(X):
    ...
    fig.add_trace(
        go.Scatter3d(x=df["x1"], y=df["x2"], z=df["x3"], mode="markers"), row=1, col=1
    ).update_traces(
...
def plot_widget():
    ...
    rotating_line = line_between_points(
        px, rotating_line_initial_state[0], rotating_line_initial_state[1]
    )
    ...
    final_data = (
        main_plot.data + pca_line_plot.data + rotating_line.data + projection_plot.data
    )
    ...
    fig = (
        go.FigureWidget(data=final_data)
        .update_yaxes(scaleanchor="x", scaleratio=1, range=[-1, 1], visible=False)
        .update_xaxes(range=[-1.5, 1.5], visible=False)
    )
    rhs_fig = (
        go.FigureWidget(data=rhs_line.data + rhs_scatter.data)
        .update_yaxes(
            scaleanchor="x", scaleratio=1, range=[-1, 1], showgrid=False, visible=False
        )
    ...
    rhs_fig.update_layout(
        dict(
            width=500, height=400, plot_bgcolor="rgba(0,0,0,0)", title="PCA Projection"
        )
    )
