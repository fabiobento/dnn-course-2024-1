...
class plt_one_addpt_onclick:
    """classe para executar um gráfico interativo"""
    ...
    def __init__(self, x, y, w, b, logistic=True):
        ...
        if self.logistic:
            ...
            ax.set_title("Exemplo de regressão linear em dados categóricos")

        ax.text(
            0.65,
            0.8,
            "[Clique para adicionar pontos de dados]",
            size=10,
        ...
        if self.logistic:
            self.bcalc = Button(
                axcalc, "Executar regressão logística (clique)", color=dlblue
            )
            self.bcalc.on_clicked(self.calc_logistic)
        else:
            self.bcalc = Button(
                axcalc, "Executar regressão linear (clique)", color=dlblue
            )
            self.bcalc.on_clicked(self.calc_linear)
        self.bthresh = CheckButtons(
            axthresh, ("Alternar o limite de 0,5 (após a regressão)",)
        )
