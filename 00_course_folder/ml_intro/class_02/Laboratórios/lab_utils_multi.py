...
def plt_house_x(X, y, f_wb=None, ax=None):
    ...
    ax.set_title("Preços de Casas")
    ax.set_ylabel("Preço (em 1000s de dólares)")
    ax.set_xlabel(f"Tamanho (1000 sqft)")
    if f_wb is not None:
        ax.plot(X, f_wb, c=dlblue, label="Nossa Predição")
    ax.legend()
...
def plt_equal_scale(X_train, X_norm, y_train):
    ...
    plt_contour_multi(
        ...
        0,
        1,
        title="J(w,b), vs w[0],w[1] não-normalizados",
        xlabel="w[0] (tamanho(sqft))",
        ylabel="w[1] (# quartos)",
    )
    #
    ...
    plt_contour_multi(
        ...
        0,
        1,
        title="J(w,b), vs w[0],w[1] normalizados",
        xlabel="w[0] (tamanho normalizado(sqft))",
        ylabel="w[1] (# de quartos normalizado)",
    )
    fig.suptitle("Contorno de custo com escala igual", fontsize=18)
...
def plt_divergence(p_hist, J_hist, x_train, y_train):
    ...
    fig.suptitle(f"O custo aumenta quando a taxa de aprendizado é muito grande")
    # ===============
    #  First subplot
    # ===============
    ax = fig.add_subplot(
    ...
    ax.set_title("Custo vs w, b fixo em 100")
    ax.set_ylabel("Custo")
    ax.set_xlabel("w")
...
def plt_gradients(x_train, y_train, f_compute_cost, f_compute_gradient):
    ...
    ax[0].set_title("Custo vs w, com gradiente; b fixo em 100")
    ...
    ax[1].set_title("Gradiente mostrado em um gráfico quiver")
    Q = ax[1].quiver(
        X,
        Y,
        U,
...
def plot_cost_i_w(X, y, hist):
    ...
    ax[0].set_title("Custo vs Iteração")
    ax[0].set_xlabel("iteração")
    ax[0].set_ylabel("custo")
    ax[1].plot(wr, cst)
    ax[1].set_title("Custo vs w[0]")
    ax[1].set_xlabel("w[0]")
...
def gradient_descent_houses(
    ...
    print(
        f"Iteração custo          w0       w1       w2       w3       b       djdw0    djdw1    djdw2    djdw3    djdb  "
    )
    print(
        f"---------------------|--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|"
    )
...
def run_gradient_descent(X, y, iterations=1000, alpha=1e-6):
    ...
    print(f"w,b encontrados pelo gradiente descendente: w: {w_out}, b: {b_out:0.2f}")
