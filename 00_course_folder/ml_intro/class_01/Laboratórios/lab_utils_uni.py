...
def plt_intuition(x_train, y_train):
    ...
    def func(w=150):
        ...
        ax[1].scatter(
            w, cur_cost, s=100, color=dldarkred, zorder=10, label=f"custo em w={w}"
        )
        ax[1].hlines(
        ...
        ax[1].set_title("Custo vs. w, (b fixado em 100)")
        ax[1].set_ylabel("Custo")
        ax[1].set_xlabel("w")
        ax[1].legend(loc="upper center")
        fig.suptitle(f"Minimize o custo: Custo atual = {cur_cost:0.0f}", fontsize=12)
        plt.show()
...
def plt_stationary(x_train, y_train):
    ...
    plt.title("Cost(w,b) \n [Você pode rotacionar essa figura]", size=12)
...
def soup_bowl():
    ...
    ax.set_title("$J(w,b)$\n [Você pode rotacionar essa figura]", size=15)
